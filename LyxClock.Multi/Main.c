...
// Buttons
static bit bPressedS1, bPressedS2, bSync;
...
void OnTimer () interrupt 1
{
	...
	if (nCurrentTime.Second != nLastSec)
	{
		...
		if (bMaster && nCurrentTime.Second == 0 && nMode == MODE_CLOCK)
		{
			//Send out message
			KEY_SYNC = 0;
			bSync = 1;
...
void AdjustTime (bit bIncrease)
{
	...
	case MODE_BEEP:
		bBeep = !bBeep;
		DS1302WriteMyData ((bMaster ? 0x2 : 0) | (bBeep));
		bBlockClock = 0;
		return;
	case MODE_MASTER:
		bMaster = !bMaster;
...
{
	bBlockClock = 1;
	//DS1302ReadTime (&nCurrentTime);	//Read Time
	if (nCurrentTime.Second == 0 && n100Ms < 1)
	{
		bBlockClock =0;
		return;
	}

	if (nCurrentTime.Second < 0x30)
		nCurrentTime.Second = 0;
	else if (nCurrentTime.Minute < 0x59)
	{
		nCurrentTime.Second = 0;
		nCurrentTime.Minute = BCDInc (nCurrentTime.Minute, 0x60, 0);
	}
	else
	{
		bBlockClock =0;
		return;	//Ignore, wait for next minutes, as increase one minutes may increase hours/day/year...
	}
